...
function Paragrph({ children }: PropsWithChildren) {
...
const statusStyles = {
	viable: {
		...
		className: 'bg-amber-100 text-green-700 dark:bg-freen-900/30 dark:text-green-400',
	},
	needs_adjustment: {
...
export function Content({ insight }: ContentProps) {
	...
	return (
		<div className="lg:scrollar-thin lg:max-h-93 lg:overflow-y-auto lg:pr-2 lg:[scrollbar-color:var(--border)_transparent]">
			...
			<section>
				...
				<Paragrph>{insight.diagnosis.content}</Paragrph>
			</section>
			<section>
			...
			<section>
				...
				<Paragrph>{insight.motivation.content}</Paragrph>
			</section>
		</div>
	);
}
